...
use common::{has_elevated_privileges, init, run_prtip};
...
fn test_fast_scan() {
    ...
    if output.status.success() {
        ...
        assert!(!stdout.is_empty(), "Fast scan should produce output");
    }
}
...
fn test_scan_with_timeout() {
    ...
    let output = run_prtip(&["-sT", "-p", "80", "--timeout", "100", "127.0.0.1"]);
...
fn test_scan_rate_limiting() {
    ...
    let output = run_prtip(&["-sT", "-p", "80-85", "--rate", "10", "127.0.0.1"]);
