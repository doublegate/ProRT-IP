//! Top ports database for fast scanning
//!
//! Provides pre-computed lists of the most commonly used ports based on
//! nmap-services frequency data. This enables fast scanning with `-F` flag
//! and `--top-ports` option.

/// Top 100 most common ports (nmap default for `-F`)
///
/// Based on nmap-services frequency data. These ports are most likely
/// to be open on typical Internet hosts.
...
/// Top 1000 ports for future use
///
/// Full nmap-services top 1000 ports. Currently only first 100 are used,
/// but full list is available for v0.4.0+ compatibility improvements.
...
/// Get top N ports from the database
///
/// Returns a vector of the N most commonly used ports. If N is greater
/// than the number of ports in our database, returns all available ports
/// plus sequential ports to reach N.
///
/// # Arguments
///
...
/// ```
/// // Get top 10 ports
/// let ports = prtip_core::top_ports::get_top_ports(10);
/// assert_eq!(ports.len(), 10);
/// assert!(ports.contains(&22));  // SSH (in top 10)
/// assert!(ports.contains(&53));  // DNS (in top 10)
///
/// // Get top 100 (fast scan)
/// let ports = prtip_core::top_ports::get_top_ports(100);
/// assert_eq!(ports.len(), 100);
/// assert!(ports.contains(&80));  // HTTP (in top 100)
/// assert!(ports.contains(&443)); // HTTPS (in top 100)
/// ```
pub fn get_top_ports(n: usize) -> Vec<u16> {
    ...
    if n <= TOP_100_PORTS.len() {
        // Fast path: top 100
        TOP_100_PORTS[..n].to_vec()
    } else if n <= TOP_1000_PORTS.len() {
        // Medium path: top 1000
        TOP_1000_PORTS[..n].to_vec()
    } else {
        // Fallback: return all we have + sequential ports
        // Note: This is not ideal, but handles edge cases
        // Most users will use 100 or 1000, not larger values
        let mut ports = TOP_1000_PORTS.to_vec();

        // Add sequential ports starting from 1001
        for port in (TOP_1000_PORTS.len() as u16 + 1)..=65535 {
...
///
/// Used to convert top ports list into a format compatible with PortRange::parse()
///
/// # Examples
///
/// ```
...
mod tests {
    ...
    #[test]
    fn test_top_100_ports_defined() {
        assert_eq!(TOP_100_PORTS.len(), 100);
        // Verify common ports are present
        assert!(TOP_100_PORTS.contains(&80)); // HTTP
        assert!(TOP_100_PORTS.contains(&443)); // HTTPS
        assert!(TOP_100_PORTS.contains(&22)); // SSH
        assert!(TOP_100_PORTS.contains(&21)); // FTP
        assert!(TOP_100_PORTS.contains(&25)); // SMTP
        assert!(TOP_100_PORTS.contains(&53)); // DNS
    }

    #[test]
    fn test_top_1000_ports_defined() {
        assert_eq!(TOP_1000_PORTS.len(), 1000);
        // Verify it's a superset of top 100
        for port in TOP_100_PORTS {
            assert!(TOP_1000_PORTS.contains(port));
        }
    }

    #[test]
    fn test_get_top_ports_zero() {
        let ports = get_top_ports(0);
        assert_eq!(ports.len(), 0);
    }

    #[test]
    fn test_get_top_ports_small() {
        let ports = get_top_ports(10);
        assert_eq!(ports.len(), 10);
        // Should be first 10 from TOP_100
        assert_eq!(ports, &TOP_100_PORTS[..10]);
    }

    #[test]
    fn test_get_top_ports_100() {
        let ports = get_top_ports(100);
        assert_eq!(ports.len(), 100);
        assert_eq!(ports, TOP_100_PORTS);
    }

    #[test]
    fn test_get_top_ports_500() {
        let ports = get_top_ports(500);
        assert_eq!(ports.len(), 500);
        // Should be first 500 from TOP_1000
        assert_eq!(ports, &TOP_1000_PORTS[..500]);
    }

    #[test]
    fn test_get_top_ports_1000() {
        let ports = get_top_ports(1000);
        assert_eq!(ports.len(), 1000);
        assert_eq!(ports, TOP_1000_PORTS);
    }

    #[test]
    fn test_get_top_ports_excessive() {
        let ports = get_top_ports(2000);
        assert_eq!(ports.len(), 2000);
        // Should contain all of TOP_1000 plus more
    ...
    #[test]
    fn test_common_ports_in_top_100() {
        // Verify most important ports are in top 100
        // Note: Not all "common" ports make the top 100 by frequency
        // Only checking truly essential ports
        let essential_ports = [21, 22, 23, 25, 53, 80, 110, 143, 443, 445, 3389, 8080];
        for port in &essential_ports {
            assert!(
                TOP_100_PORTS.contains(port),
                "Port {} should be in TOP_100",
                port
            );
        }
    }
