//! macOS packet capture implementation using BPF
...
/// macOS-specific packet capture using BPF devices
...
impl MacOSCapture {
    /// Create a new macOS packet capture instance
    pub fn new() -> Self {
        Self {
            interface: None,
            tx: None,
...
impl PacketCapture for MacOSCapture {
    fn open(&mut self, interface: Option<&str>) -> Result<()> {
        ...
        tracing::info!("Opening BPF on macOS. You may need root privileges or ChmodBPF installed.");
            ...
            Err(e) => {
                let err_msg = format!(
                    "Failed to create channel on {}: {}. \
                     Ensure you have root privileges or ChmodBPF is installed.",
                    iface.name, e
                );
        ...
        tracing::info!("Packet capture opened successfully on macOS");
        Ok(())
    }
...
mod tests {
    ...
    fn test_get_interfaces() {
        let interfaces = MacOSCapture::get_interfaces();
