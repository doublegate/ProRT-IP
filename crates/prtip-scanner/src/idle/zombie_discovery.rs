...
impl ZombieDiscovery {
    ...
    pub async fn find_zombies(&self) -> Result<Vec<ZombieCandidate>> {
        // Parse subnet
        let network = self.subnet.parse::<IpNetwork>()
    ...
    async fn test_candidate(&self, ip: IpAddr) -> Result<ZombieCandidate> {
        ...
        let pattern = tracker.classify_pattern(self.config.probes_per_host).await?;
        
        // Calculate latency
        let latency_ms = start_time.elapsed().as_millis() as u64 / self.config.probes_per_host as u64;
