...
impl PortSpec {
    ...
    /// The first 100 ports of ProRT-IP's priority list -- the same set `-F` uses.
    ///
    /// The ordering is an editorial ranking of IANA port assignments, not a
    /// frequency ranking. See [`prtip_core::top_ports`].
    pub fn top_100() -> Self {
        Self {
            ports: prtip_core::top_ports::PRIORITY_PORTS_100
                .iter()
                .copied()
                .collect(),
        }
    ...
    /// The first 1000 ports of the same list -- what `--top-ports 1000` uses.
    pub fn top_1000() -> Self {
        Self {
            ports: prtip_core::top_ports::PRIORITY_PORTS_1000
                .iter()
                .copied()
                .collect(),
        }
...
pub fn handle_port_selection_event(event: &Event, state: &mut PortSelectionState) -> bool {
    ...
    }) = event
    {
        match (*code, *modifiers) {
            ...
            // Escape - unfocus input; when not focused, fall through so the
            // parent handles it (the `_` arm below returns false).
            (KeyCode::Esc, KeyModifiers::NONE) if state.input_focused => {
...
mod tests {
    ...
    fn test_port_spec_top_1000() {
        ...
        // Exactly the crate's 1000-port priority list, no padding of its own.
        assert_eq!(spec.count(), 1000);
        assert!(spec.contains(80));
        assert!(spec.contains(443));
        // The 100-preset must be a subset of the 1000-preset.
        for port in PortSpec::top_100().ports.iter() {
            assert!(spec.contains(*port), "port {} missing from top_1000", port);
        }
    }
