...
- **Priority Ports:** Fast scanning of a fixed, IANA-derived priority list (first 100, first 1000, or custom N) -- an editorial ranking, not a frequency ranking
...
**Common Use Cases:**
- **Quick Scan:** `-F` or `--top-ports 100` (first 100 of the priority list)
...
### Priority Ports (`-F` / `--top-ports`)

Scan the first N ports of ProRT-IP's priority list:

```bash
# First 100 ports of the priority list (fast scan)
prtip -F 192.168.1.1
...
**What the priority list is — read this before relying on it**

The ordering is an **editorial ranking of IANA port assignments**. It is produced
by applying a written rule ([`tools/gen-top-ports/RULE.md`][rule]) to a committed
snapshot of the IANA Service Name and Transport Protocol Port Number Registry.
The rule groups each assignment into one of thirteen service families using the
registry's own service names and descriptions, ranks those families by how much
an operator learns from finding one open, and interleaves them so that any prefix
of the list spans every family. Ties are broken by ascending port number.

**It is not a frequency ranking.** No input to it is a measurement. Therefore:

- `-F` and `--top-ports N` select a **different set of ports** than Nmap, Masscan,
  RustScan or any other scanner whose list derives from measured frequency data.
  Results are not comparable port-for-port, and a script that assumed `-F` would
  reach a particular port should name that port with `-p`.
- ProRT-IP publishes **no hit-rate or coverage figure** for these flags, because
  it has measured none. Any number you may remember from an older version of this
  page was not measured by this project.

The reason is licensing. `nmap-services` is covered by the Nmap Public Source
License, which makes any work that reads it a Derivative Work that may not be
distributed under plain GPL-3.0. scans.io and Censys both restrict their data to
non-commercial use, which a GPL-3.0 work cannot carry because GPL-3.0 guarantees
commercial use and redistribution. With no lawfully redistributable
measured-frequency dataset available, ProRT-IP ships an ordering it can justify
from first principles rather than one it cannot ship.

For context on how much a frequency ordering would have bought: Izhikevich,
Teixeira and Durumeric, [*LZR: Identifying Unexpected Internet
Services*][lzr] (USENIX Security 2021), found that only about **3% of HTTP
services run on port 80**. Port-based assumptions are a weaker heuristic than
their reputation suggests, whatever orders the list.

**Common values:**

```bash
# Ultra-fast reconnaissance (first 10)
prtip --top-ports 10 TARGET
# Ports: 22, 80, 20, 65, 25, 42, 31, 111, 537, 119

# Quick scan (first 100) - DEFAULT -F behavior
prtip -F TARGET
prtip --top-ports 100 TARGET

# Thorough scan (first 1000)
prtip --top-ports 1000 TARGET
```

**The first 20 entries**, one port per family in family order, repeating:

| # | Port | Family | IANA service |
|---|------|--------|--------------|
| 1 | 22 | remote-access | `ssh` |
| 2 | 80 | web | `http` |
| 3 | 20 | file-sharing | `ftp-data` |
| 4 | 65 | database | `tacacs-ds` (registry description: "TACACS-Database Service") |
| 5 | 25 | mail | `smtp` |
| 6 | 42 | name-service | `name` (Host Name Server) |
| 7 | 31 | directory-auth | `msg-auth` |
| 8 | 111 | management-discovery | `sunrpc` |
| 9 | 537 | voice-video | `nmsp` (Networked Media Streaming Protocol) |
| 10 | 119 | messaging | `nntp` |
| 11 | 92 | print | `npp` (Network Printing Protocol) |
| 12 | 67 | network-infra | `bootps` |
| 13 | 23 | remote-access | `telnet` |
| 14 | 280 | web | `http-mgmt` |
| 15 | 21 | file-sharing | `ftp` |
| 16 | 66 | database | `sql*net` |
| 17 | 50 | mail | `re-mail-ck` |
| 18 | 43 | name-service | `nicname` (WHOIS) |
| 19 | 49 | directory-auth | `tacacs` |
| 20 | 123 | management-discovery | `ntp` |

Reproduce this at any length with
`python3 tools/gen-top-ports/generate.py --explain 20`.

Two consequences worth seeing plainly: port 65 outranks port 3306 because the
registry describes it as a "Database Service" and the rule classifies from that
text; and ports 3389 (RDP) and 8080 (HTTP-alt) land at positions **309** and
**532**, not in the first hundred, because within a family the well-known range
is walked before the registered range. A frequency-ordered list would place both
early. This one does not, and that is the visible cost of having no frequency
data. Both are inside `--top-ports 1000`.

**Full list:** `crates/prtip-core/src/top_ports.rs`, constants
`PRIORITY_PORTS_100` and `PRIORITY_PORTS_1000`.

**Performance Comparison:**

| Specification | Ports Scanned | Typical Duration |
|---------------|---------------|------------------|
| `--top-ports 10` | 10 | 1-2 seconds |
| `-F` (first 100) | 100 | 3-5 seconds |
| `--top-ports 1000` | 1,000 | 30-60 seconds |
| `-p 1-1000` | 1,000 | 30-60 seconds |
| `-p-` (all) | 65,535 | Hours |

There is no coverage column. Only the `-p-` row scans every port, and only it can
...
- ✅ **Continuous monitoring:** periodic scans of a fixed port set
- ❌ **Anything where a specific port must be reached:** name it with `-p`
- ❌ **Compliance audits:** may require a full port scan (`-p-`)
- ❌ **Exhaustive security testing:** hidden services sit on non-standard ports

**Recommendation:**
Use `-F` when scan time is the constraint and the exact port set is not. Use `-p`
whenever a particular service matters, and `-p-` when completeness matters.

[rule]: https://github.com/doublegate/ProRT-IP/blob/main/tools/gen-top-ports/RULE.md
[lzr]: https://www.usenix.org/conference/usenixsecurity21/presentation/izhikevich
...
1. **Quick scan** (`-F`): 100 ports in seconds -- a fixed port set, not a coverage guarantee
...
**Rationale:**
- `-F` covers its own 100-port list in seconds; it makes no claim about the rest of the host
...
**Related Files:** `crates/prtip-core/src/types.rs` (PortRange), `crates/prtip-core/src/top_ports.rs` (PRIORITY_PORTS_100/PRIORITY_PORTS_1000), `tools/gen-top-ports/RULE.md` (the ranking rule)
