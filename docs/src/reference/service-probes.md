...
- **37 service probes** (25 TCP, 12 UDP) covering HTTP, SSH, FTP, SMTP, databases, and more
- **226 match rules** (183 `match` + 43 `softmatch`) for version identification
- **ProRT-IP's own clean-room probe format**, documented in `tools/gen-service-probes/FORMAT.md`
...
ProRT-IP uses an embedded service probe corpus, generated by `tools/gen-service-probes/generate.py` from IANA port assignments and published protocol specifications:

| Metric | Value | Details |
|--------|-------|---------|
| **Total Size** | ~20 KB | Complete corpus embedded at compile time |
| **Service Probes** | 37 probes (25 TCP, 12 UDP) | TCP and UDP probes for common services |
| **Match Rules** | 226 (183 match + 43 softmatch) | Regex signatures for version identification |
| **Ports Covered** | 96 distinct ports | Port-specific probes; a young, growing corpus |
| **Format** | ProRT-IP service probe format | Documented in `tools/gen-service-probes/FORMAT.md` |
...
| **1** | Very light | Rarity 1 probes | 5-15s | Common services only |
| **2** | Light | Rarity 1-2 probes | 10-30s | Fast scan, limited accuracy |
| **3** | Conservative | Rarity 1-3 probes | 20-60s | Balanced speed |
| **4** | Moderate | Rarity 1-4 probes | 30-90s | Good coverage |
| **5** | Standard | Rarity 1-5 probes | 45-120s | Comprehensive |
| **6** | Thorough | Rarity 1-6 probes | 60-150s | High accuracy |
| **7** | Recommended | Rarity 1-7 probes | 75-180s | Default, best balance |
| **8** | Aggressive | Rarity 1-8 probes | 90-240s | Near-complete |
| **9** | Maximum | All 37 probes | 120-300s | Exhaustive detection |
...
**Intensity 7:** Rarity 1-7 (recommended, broad coverage of the corpus)
...
Scan time scales with the number of probes tried at each intensity level
(detection rate is not yet measured):

```
Intensity 1: fewest probes  × 1s timeout ≈ a few seconds
Intensity 3: more probes    × 1s timeout ≈ tens of seconds
Intensity 7: most probes    × 1s timeout ≈ default balance
Intensity 9: all 37 probes  × 1s timeout ≈ 37s scan time (exhaustive)
```
...
To contribute new probes or signatures to ProRT-IP's own corpus:

**1. Clone ProRT-IP and locate the overlay:**
```bash
git clone https://github.com/doublegate/ProRT-IP.git
cd ProRT-IP
```

**2. Edit `tools/gen-service-probes/overlay.txt`:**
```bash
vim tools/gen-service-probes/overlay.txt
# Add your probe definition (see FORMAT.md for the grammar)
```

**3. Regenerate and test:**
```bash
python3 tools/gen-service-probes/generate.py
cargo test -p prtip-core --lib service_db
```

**4. Submit Pull Request:**
- Include test results
- Cite the published protocol specification, vendor documentation, or
  first-hand observation the probe/match rules are derived from (see
  `tools/gen-service-probes/README.md` — content copied from
  `nmap-service-probes` or `nmap-os-db` is not acceptable, GPL-3.0
  incompatible)
